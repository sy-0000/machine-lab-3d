...
for(const machine of MACHINES)test(`${machine.id}: real GLB + supplied JSON, pivots, feed, ownership, spindle, reset`,async()=>{
 ...
 try{
  ...
  if(machine.id==='lathe'){toggleDemoWorkpiece(m,false);setAxis(m,'x',-.14);stepMachine(m,true,500,2);assert.equal(safetyStatus(m).level,'danger');const gap=safetyStatus(m).gap;m.scene.position.add(new Vector3(5,2,1));near(safetyStatus(m).gap,gap);}
  ...
  if(machine.id==='lathe'){resetMachine(m);const lower=world(m.lookup.Object_103),base=world(m.lookup.Object_101);setAxis(m,'z',.04);near(world(m.lookup.Object_103).y-lower.y,.04);near(world(m.lookup.Object_101).distanceTo(base),0);const q=m.lookup.ToolIndexPivot.quaternion.clone();indexTool(m);near(q.angleTo(m.lookup.ToolIndexPivot.quaternion),Math.PI/18);for(let i=0;i<35;i++)indexTool(m);near(q.angleTo(m.lookup.ToolIndexPivot.quaternion),0);stepMachine(m,true,500,2);const angle=m.spindleAngle;emergencyStop(m);near(m.rpm,500);assert.equal(m.emergency,false);stepMachine(m,false,500,.1);assert.ok(m.rpm>0&&m.rpm<500);assert.ok(m.spindleAngle>angle);stepMachine(m,false,500,1);near(m.rpm,0);stepMachine(m,true,500,2);near(m.rpm,500);}
  if(machine.id==='drill'){resetMachine(m);const bit=m.lookup.TwistDrill,before=world(bit);setAxis(m,'quill',-.04);near(world(bit).y-before.y,-.04);const table=m.lookup.TableAssembly,q=orientation(table);stepMachine(m,true,500,1);assert.ok(orientation(table).angleTo(q)<1e-6);turnControl(m,'feed',-1,.4);assert.ok(m.offsets.quill<0);for(let i=0;i<120;i++)stepReturn(m,null,1/60);near(m.offsets.quill,0);near(m.angles.feed,0);setAxis(m,'table',.1);assert.ok(m.offsets.table>0);}
 }finally{disposeMachine(m);}
});
test('invalid JSON axes fail explicitly',async()=>{const raw=JSON.parse(readFileSync(new URL('../../public/models/drill_press_parts.json',import.meta.url)));raw.interactions.FeedHandlePivot.axis=[0,0,0];const {scene}=await loadSource(new URL('../../public/models/machines/drill/drill.glb',import.meta.url));assert.throws(()=>prepareMachine(scene,normalizeConfig('drill',raw)),/無效旋轉／移動軸/);});
test('shared runtime animation is frame independent at 30 and 144 FPS',async()=>{
 const raw=JSON.parse(readFileSync(new URL('../../public/models/drill_press_parts.json',import.meta.url)));const {scene}=await loadSource(new URL('../../public/models/machines/drill/drill.glb',import.meta.url));const m=prepareMachine(scene,normalizeConfig('drill',raw));
...
test('missing referenced spindle child is reported rather than inferred',async()=>{
 const raw=JSON.parse(readFileSync(new URL('../../public/models/drill_press_parts.json',import.meta.url)));const {scene}=await loadSource(new URL('../../public/models/machines/drill/drill.glb',import.meta.url));scene.getObjectByName('Chuck').name='UnidentifiedChuck';const m=prepareMachine(scene,normalizeConfig('drill',raw));assert.ok(m.errors.includes('找不到 JSON 節點：Chuck'));disposeMachine(m);
});
