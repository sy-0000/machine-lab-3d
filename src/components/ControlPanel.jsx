...
export default function ControlPanel({controls:c,model}){
 ...
 return <aside className="panel"><span className="eyebrow">MACHINE CONTROL</span><h2>操作控制台</h2><fieldset disabled={!model}>
  ...
  {model?.config.actions?.filter(a=>a.type==='detent').map(a=><label key={a.id}>{a.label}<select aria-label={a.label} value={model.detents[a.id]} onChange={e=>c.detent(a.id,Number(e.target.value))}>{a.degrees.map((angle,index)=><option key={angle} value={index}>{a.id===model.config.speedSelectors?.gear?`${model.config.speedSelectors.baseRpm[index]*model.config.speedSelectors.multipliers[model.detents[model.config.speedSelectors.mode]]} RPM`:a.id===model.config.speedSelectors?.mode?model.config.speedSelectors.modeLabels[index]:`${angle}°`}</option>)}</select></label>)}
  <label className="range-label" htmlFor="rpm">設定轉速 <output>{c.rpm} RPM</output></label><input hidden={!!model?.config.speedSelectors} disabled={!!model?.config.speedSelectors} id="rpm" type="range" min="0" max={model?.config.maxRpm||2000} step="10" value={c.rpm} onChange={e=>c.setRpm(e.target.value)}/>
  ...
  {model?.config.actions?.some(a=>a.type==='emergency')&&<button onClick={c.brake}>腳踏煞車（減速停止）</button>}
