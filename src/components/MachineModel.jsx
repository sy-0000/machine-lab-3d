...
export default function MachineModel({model,controls,orbit}) {
 ...
 useEffect(()=>{
  ...
  function hit(e){const rect=canvas.getBoundingClientRect();point.set((e.clientX-rect.left)/rect.width*2-1,1-(e.clientY-rect.top)/rect.height*2);model.scene.updateWorldMatrix(true,true);camera.updateMatrixWorld();ray.setFromCamera(point,camera);return controlFor(ray.intersectObject(model.scene,true)[0]?.object);}
  function down(e){
