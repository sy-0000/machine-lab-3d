...
import {prepareMachine,disposeMachine,resetMachine,setAxis,turnControl,stepMachine,safetyStatus,setHighlight,toggleDemoWorkpiece} from './runtime.js';
...
for(const machine of MACHINES)test(`${machine.id}: real GLB + supplied JSON, pivots, feed, ownership, spindle, reset`,async()=>{
 ...
 try{
  assert.deepEqual(m.errors,machine.id==='milling'?['找不到 JSON 節點：Worktable_X_Slide']:[]);
  ...
  for(const wheel of m.config.wheels){
   ...
   turnControl(m,wheel.id,-1,.2,true);assert.ok(m.angles[wheel.id]<0);assert.ok(m.offsets[wheel.drives]<0);
   ...
   turnControl(m,wheel.id,1,.2,true);near(m.angles[wheel.id],0);
  ...
  if(machine.id==='milling'){resetMachine(m);const work=m.lookup.Workholding,before=world(work);setAxis(m,'X_Axis_Table',.2);near(world(work).z-before.z,.2);}
  if(machine.id==='drill'){resetMachine(m);const bit=m.lookup.DrillBit,before=world(bit);setAxis(m,'quill',-.04);near(world(bit).y-before.y,-.04);const table=m.lookup.TableAssembly,q=orientation(table);stepMachine(m,true,500,1);assert.ok(orientation(table).angleTo(q)<1e-6);}
