...
for(const machine of MACHINES){
 ...
 if(machine.id==='milling'){const manifest=JSON.parse(readFileSync(new URL('../assets-src/milling/milling_machine_split_manifest.json',import.meta.url)));const names=new Set(json.nodes.map(n=>n.name));manifestAudit={count:manifest.parts.length,missing:manifest.parts.filter(p=>!names.has(p.name)).map(p=>p.name)};if(manifestAudit.missing.length)throw Error('Manifest references missing GLB nodes');}
