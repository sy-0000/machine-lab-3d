...
import { loadBestStars } from '../levels/progress.js';
import { Stars } from '../components/level/LevelParts.jsx';

const MACHINE = { lathe: '車床', milling: '銑床', drill: '鑽床' };

export default function LevelsPage() {
  const best = loadBestStars(), total = CHALLENGES.reduce((n, c) => n + (best[c.id] || 0), 0);
  return <main className="info-page levels-page">
    <div className="info-header"><a className="back-link" href="#/">← 返回首頁</a><span className="eyebrow green">加工挑戰</span><h1>加工關卡</h1>
      <p className="subtitle">三台機台各一關，每關 5–8 分鐘。車到指定尺寸後繳交，依誤差得到 1–3 顆星。</p>
      <p className="star-total" aria-label={`已獲得 ${total} / ${CHALLENGES.length * 3} 顆星`}><span aria-hidden="true">★</span> {total} / {CHALLENGES.length * 3}</p></div>
    <ol className="challenge-cards">
      {CHALLENGES.map((c, i) => <li key={c.id} className="challenge-card">
        <div className="challenge-card-top"><span className="node-dot">{i + 1}</span>
          {c.id in best ? <Stars count={best[c.id]} small /> : <small className="not-played">尚未挑戰</small>}</div>
        ...
        <a className="level-action primary-link" href={'#/challenge/' + c.id}>{c.id in best ? '再挑戰 →' : '開始 →'}</a>
      </li>)}
    </ol>
