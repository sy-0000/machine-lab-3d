...
export default function ControlPanel({controls:c,model}){
 ...
 return <aside className="panel"><span className="eyebrow">MACHINE CONTROL</span><h2>操作控制台</h2><fieldset disabled={!model}>
  <div className="button-row"><button className="primary" onClick={c.start} disabled={c.running||!model?.pivots[model?.config.spindle.node]}>▶ 啟動主軸</button><button onClick={c.stop} disabled={!c.running}>■ 停止主軸</button></div>
