...
React、Vite、Three.js、React Three Fiber 與 Drei 製作的純前端網站，提供工安守則、刀具介紹、車床／銑床／鑽床加工教室，以及三個限時加工小關卡。套件版本為 3.0.0，模組架構名稱為 Machine System v1.0。

首頁不下載機台模型，進入加工教室後才載入。加工關卡是三個獨立小關卡（車床外徑車削 5 分鐘、銑床面銑平面 5 分鐘、鑽床定位鑽孔與攻牙 8 分鐘）：任務 → 實作 → 繳交，依量到的尺寸誤差給 0–3 顆星。實作時側欄「目前工件尺寸」隨時顯示原始毛胚與目前量到的尺寸；停機時刀具壓進工件會提示「過頭」。鑽床關卡要自己選鑽頭（Ø8.5／Ø10／M10 絲攻）並用「工件定位 X」把鑽頭對準劃線。每次開始都用新毛胚，不保留關卡進度。工作區右上角可切換全螢幕。
...
| 場景、光線、視角按鈕（全景／工作區／手輪／刀具，鍵盤 1–4） | 加工教室 | `src/components/MachineScene.jsx`、`src/machines/cameraViews.js` |
| 3D 點選、拖曳與動畫 | 加工教室 | `src/components/MachineModel.jsx` |
| 操作事件與 React 狀態 | 加工教室 | `src/hooks/useMachineControls.js` |
| 刀具與刀片幾何 | 全站 | `src/tools/toolGeometry.js` |
| 虎鉗、平行塊與接觸光圈 | 銑床／鑽床 | `src/machining/fixtures.js` |
| 全站配色、卡片與響應式版面 | 全站 | `src/styles/app.css` |

只改文案與排版，優先修改 `src/pages/` 與 CSS（`src/styles/app.css`、`src/styles/workspace.css`）。刀具介紹由刀具目錄產生，新增資料請改 `tool.config.js`。
...
三個 GLB 透過 Git LFS 儲存。車床（原始 24 MiB → 約 6 MiB）與銑床（原始 181 MiB → 約 16 MiB）的網頁版由 `assets-src/<機台>/<機台>.source.glb` 以 `npm run optimize:models` 產生（meshopt 幾何壓縮＋WebP 貼圖，保留所有節點與材質名稱）；改原始模型後重新執行，不要手動編輯網頁版。
...
- `assets-src/`：車床、銑床原始模型與銑床節點 manifest；不會部署，模型稽核仍使用 manifest。

掛載設定的 `position` 是相對父節點的局部座標，不能直接填世界座標。修改機械資料後需跑測試與模型稽核。

## 操作與驗證

空白處拖曳旋轉視角、滾輪縮放、右鍵拖曳平移；視角按鈕或鍵盤 1–4 切到全景／工作區／手輪／刀具。操作面板每按一下移動選定的進給量（0.01–5 mm），按住會連續轉動手輪。刀尖碰到工件時工件上出現光圈（琥珀色＝接觸，橘色＝切削中）。工件只能在主軸停止且啟動桿回位後裝卸。重設會恢復操作位置與相機。
...
開發網址加上 `?inspect=1` 可啟用診斷，例如 `http://localhost:5173/?inspect=1#/lathe`。`reports/` 是測試截圖與稽核輸出（不提交），`tests/` 放瀏覽器測試。`dist/`、依賴與測試暫存不提交。
